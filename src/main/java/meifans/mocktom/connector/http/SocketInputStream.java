...
public class SocketInputStream extends InputStream {
    ...
    private void readMethodName(final HttpRequestLine requestLine) throws IOException {
        ...
        while (!space) {
            if (readCount >= maxRead) {
                ...
                tryResize((r) -> resizeMethod(r), requestLine, resizeable);
            }
            tryFillBuff();
            if (buf[pos] == SP) {
                space = true;
    ...
    private void resizeMethod(HttpRequestLine requestLine) {
        requestLine.method = Arrays.copyOf(requestLine.method, 2 * requestLine.method.length);
    }
    ...
    private void resizeUri(HttpRequestLine requestLine) {
        requestLine.uri = Arrays.copyOf(requestLine.uri, 2 * requestLine.uri.length);
    }
    ...
    private void resizeProtocol(HttpRequestLine requestLine) {
       requestLine.protocol =Arrays.copyOf(requestLine.protocol,requestLine.protocol.length);
    }
